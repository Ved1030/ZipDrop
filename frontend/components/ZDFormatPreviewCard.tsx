...
interface Props {
  file: any;
  onEditClick: () => void; // kept for API compatibility — no longer used
}
...
export default function ZDFormatPreviewCard({ file, onEditClick }: Props) {
  ...
  const getFormatLabel = () => {
    ...
    if (
      ["py", "js", "ts", "jsx", "tsx", "html", "css", "json"].includes(e || "")
    )
      return "Code Source";
    return "Document";
  };
  ...
  const renderSnippet = () => {
    ...
    if (e === "pdf")
      return (
        <p style={{ fontSize: "11px", color: "var(--cyan)", marginTop: "8px" }}>
          PDF · open to view
        </p>
      );
  ...
  return (
    ...
    >
      ...
      >
        {editLoading ? "Opening editor…" : "Edit before forwarding →"}
      </button>
    </div>
  );
}
